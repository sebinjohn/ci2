...
* ``ci-publish-site.bsh`` - commit a site's directory as the sole content of a given branch
* ``ci-push-branch.bsh`` - push a branch to the github repo
...
* ``sbt-ci-build-doc.bsh`` - uses sbt to build documentation for the project and commit to ``gh-pages``
...
6. [Optional to publish documentation] **For public repos only** Create a branch called `gh-pages` for the project and push it to github. Then add the private key for omnia-bamboo as an encrypted file.
   ...
   1. For sbt builds add `ci/sbt-build-doc.sh && ci/sbt-push-branch.sh gh-pages` to the build commands.
